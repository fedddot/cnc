...
ServerDataReceiver::ServerDataReceiver(const std::vector<char>& header, const std::size_t& length_field_size, const std::size_t& max_data_size, const std::string& port_path): DataReceiver(header, length_field_size, max_data_size), m_port_path(port_path), m_port_fd(open_port(port_path)), m_polling_timeout(POLLING_TIMEOUT) {
	config_port(m_port_fd);
	m_polling_thread_run_permission.store(true, std::memory_order_release);
	m_polling_thread = std::thread(&ServerDataReceiver::poll_fd, this);
}
...
int ServerDataReceiver::open_port(const std::string& port_path) {
...
void ServerDataReceiver::config_port(int port_fd) {
	if (BAD_FD == port_fd) {
...
void ServerDataReceiver::close_port(int port_fd) {
	if (0 != close(port_fd)) {
		throw std::runtime_error("failed to close the port");
	}
}

void ServerDataReceiver::poll_fd() {
